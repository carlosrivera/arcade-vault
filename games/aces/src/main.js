 ...
 *
 * Built inside init() rather than at module scope so the module can be
 * evaluated again for a hot swap without a second STRIKEVECTOR attaching
 * itself to the same canvas and overlay.
 *
 * @param {{renderer?: THREE.WebGLRenderer, canvas?: HTMLCanvasElement, state?: object}} ctx
 */
export function init(ctx = {}) {
  // Listeners routed through on() are detached again by dispose(); one left
  // behind would keep flying a jet that no longer exists.
  const _unbind = [];
  ...
  function showDeathScreen(reason) {
    ...
    on(overlay.querySelector('#startBtn'), 'click', restart);
    audio.setLockTone('off');
  }
  ...
  /** One frame, driven by the host. Idles until ENGAGE flips `running`. */
  function frame(dt) {
    if (!running) return;
    world.time += dt;
    ...
    for (const [fmv, alive, flames] of heatSrc) {
      ...
      for (const f of flames) {
        ...
        const plumePos = nozzle.addScaledVector(
          fmv.forward,
          fmv.controls.afterburner ? -0.15 : 0.0,
        );
  ...
  // The DOM outlives a hot swap, so an overlay hidden by the previous instance
  // would stay hidden while this one waits at `running = false` -- terrain
  // rendering behind an invisible menu, with no way to engage.
  overlay.classList.remove('hidden');

  return {
    update: (dt) => frame(dt),
    getState: () => ({ running, camMode }),
    dispose() {
